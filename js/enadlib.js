...
function JBE_DATE_FORMAT(d,f) {
  ...
  }else if(f=='MMM DD, YYYY'){
    rval=monthNames[date.getMonth()].substring(0,3) + ' ' + day + ', ' + year;  
...
function JBE_PRINTDIV(divName) {
  var pdiv=document.getElementById(divName);
  //pdiv.display.style='block';
  var printContents = pdiv.innerHTML;
  w = window.open();
...
function JBE_INCLUDE_HTML() {
  ...
  for (i = 0; i < z.length; i++) {
    ...
    file = elmnt.getAttribute("JBE-INCLUDE-HTML");
    if (file) {
      ...
      xhttp.onreadystatechange = function() {
        if (this.readyState == 4) {
          ...
          elmnt.removeAttribute("JBE-INCLUDE-HTML");
          JBE_INCLUDE_HTML();
        }
      }
